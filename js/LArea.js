    ...
    MobileArea.prototype = {
        ...
        getData: function(callback) {
            ...
            if (typeof _self.params.data == "object") {
                _self.data = _self.params.data; // 地址数据
                if (callback) {
                    callback()
                }
            } else {
        ...
        bindEvent: function() {
            ...
            function gearTouchStart(e) {
                ...
                if (top) {
                    target["o_d_" + target.code] = parseFloat(top.replace(/em/g, ""));
                } else {
            ...
            function gearTouchMove(e) {
                ...
                var f = (target["new_" + target.code] - target["old_" + target.code]) * 30 / window.innerHeight;
                target["pos_" + target.code] = target["o_d_" + target.code] + f;
                target.style["-webkit-transform"] = 'translate3d(0,' + target["pos_" + target.code] + 'em,0)';
                target.style["transform"] = 'translate3d(0,' + target["pos_" + target.code] + 'em,0)';
                target.style["-ms-transform"] = 'translate3d(0,' + target["pos_" + target.code] + 'em,0)';
                target.style["-o-transform"] = 'translate3d(0,' + target["pos_" + target.code] + 'em,0)';
                target.setAttribute('top', target["pos_" + target.code] + 'em');
            ...
            function rollGear(target) {
                ...
                target["int_" + target.code] = setInterval(function() {
                    ...
                    target.style["-o-transform"] =  'translate3d(0,' + pos + 'em,0)';
        ...
        setGearTooth: function(data) {
            ...
            if (l > 0) {
                var id = item[gearVal][this.keys['code']];
                var childData;
                switch (_self.type) {
                    case 1:
                ...
                gearChild[_self.index].style["-ms-transform"] = 'translate3d(0,' + (-gearVal * 2) + 'em,0)';
                gearChild[_self.index].style["-o-transform"] = 'translate3d(0,' + (-gearVal * 2) + 'em,0)';
        ...
        finish: function(e) {
            ...
            if(this.valueTo){
               ...
               // console.log(this.valueTo.value)
            }
           // console.log('this.valueTo:' + this.valueTo)
           // console.log('wode code:' + provinceCode +((cityCode)?(',' + cityCode):('')) + ((countyCode)?(',' + countyCode):('')))
            _self.close(e);
        },
        defaultAddress: function(e){
            var _self = this;
            var eAddress = e.split(',');
            var provinceCode = eAddress[0],
                cityCode = eAddress[1],
                countyCode = eAddress[2];
            var provinceText = '',
                cityText = '',
                countyText = '';
            _self.getData();
            var areaData  =_self.data;
            for(var i=0;i<areaData.length;i++){
                var provinceChild = '';
                if(areaData[i].code ==provinceCode){
                    provinceText =areaData[i].name;
                    provinceChild = areaData[i].child;
                    for(var m=0;m<provinceChild.length;m++){
                        var cityChild = '';
                        if(provinceChild[m].code == cityCode){
                            cityText = provinceChild[m].name;
                            cityChild = provinceChild[m].child;
                            for(var n=0;n<cityChild.length;n++){
                                if(cityChild[n].code == countyCode){
                                    countyText = cityChild[n].name;
                                }
                            }
                        }
                    }
