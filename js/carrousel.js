...
(function ($) {
    ...
    $.carrousel.prototype = {
       ...
       // 绑定滑动事件
        _swipeLeft: function (element, move) {
            var _self = this;
            var ne = MonoEvent;
            $$box = ne('.' + element.context.className);
        ...
        // 绑定滑动事件
        _swipeRight: function (element, move) {
            var _self = this;
            var ne = MonoEvent;
            $$box = ne('.' + element.context.className);
        ...
        //_callback:this.options.swiperCallback(),
        // 初始样式
        _layout: function () {
            this._setItems();
            ...
            this._swipeLeft(this.$leftItm, 'next'); // 左滑动
            ...
            this._swipeRight(this.$rightItm, 'prev'); // 右滑动
        ...
        _getCoordinates: function (position) {
            if (this.support3d) {
                switch (position) {
                    case 'outleft':
                        return {
                            '-webkit-transform': 'translateX(-' + this.CSSXout + 'px) translateZ(-312px) rotateY(45deg)',
                            '-moz-transform': 'translateX(-' + this.CSSXout + 'px) translateZ(-312px) rotateY(45deg)',
                            '-o-transform': 'translateX(-' + this.CSSXout + 'px) translateZ(-312px) rotateY(45deg)',
                            '-ms-transform': 'translateX(-' + this.CSSXout + 'px) translateZ(-312px) rotateY(45deg)',
                            'transform': 'translateX(-' + this.CSSXout + 'px) translateZ(-312px) rotateY(45deg)',
                            'opacity': 0
                        };
                        break;
                    case 'outright':
                    ...
                    case 'left':
                        return {
                            '-webkit-transform': 'translateX(-' + this.CSSX + 'px) translateZ(-311px) rotateY(25deg)',
                            '-moz-transform': 'translateX(-' + this.CSSX + 'px) translateZ(-311px) rotateY(25deg)',
                            '-o-transform': 'translateX(-' + this.CSSX + 'px) translateZ(-311px) rotateY(25deg)',
                            '-ms-transform': 'translateX(-' + this.CSSX + 'px) translateZ(-311px) rotateY(25deg)',
                            'transform': 'translateX(-' + this.CSSX + 'px) translateZ(-311px) rotateY(25deg)',
                            'opacity': 1,
                            'visibility': 'visible'
                        };
                        break;
...
*/
            ...
            if (!this.isAnim) {
                ...
                if(nowTimer){
                    console.log('myTimer:  ' + nowTimer);
                    clearTimeout(nowTimer); // 先清除定时器
                }
                ...
                this._setItems();
                // this.indexB :当前广告index， this.$currentItem: 当前ad 的dom
               // nowTimer = this.options.timeRangFetch(); // 启动新定时器
               nowTimer =  this.options.swiperCallback(this.options.timeOut, this.indexB, this.$currentItm);  // 切换执行callback
            }
        },
